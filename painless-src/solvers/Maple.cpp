...
void cbkExportClause(void * issuer, int lbd, vec<Lit> & cls)
{
	Maple* mp = (Maple*)issuer;

	if (cls.size() > mp->lbdLimit)
		return;
...
Maple::Maple(int id) : SolverInterface(id, MAPLE)
{
	...
	solver->cbkExportClause    = cbkExportClause;
	solver->cbkImportClause    = cbkImportClause;
	solver->cbkImportUnit      = cbkImportUnit;
	solver->issuer             = this;
}
